...
```mermaid
    ...
    A[🔍 Поиск отеля] --> B[🏨 Выбор номера]
    B --> C[📝 Создание бронирования]
    C --> D[✉️ Email-подтверждение]
    D --> E[💳 Оплата отционально т/к клиент может оплатить наличными]
    E --> F[🔔 Webhook от провайдера]
